# ContentPipe Architecture v0.8.0

> 最后更新: 2026-04-06
> 形态: 外部服务(Python/FastAPI) + OpenClaw Gateway 作为 LLM Runtime
...
│  │  internal msgs (LLM context) + visible msgs           │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  tools.py · call_llm()                                │  │
│  │  LLM 调用入口:llm_mode=gateway / direct              │  │
│  └──────────────────────────────────────────────────────┘  │
└────────────────────────────┬────────────────────────────────┘
                             │ HTTP POST /v1/chat/completions
                             │ X-OpenClaw-Agent-Id: contentpipe-blank
                             │ X-OpenClaw-Session-Key: contentpipe:{run_id}:{node}:main
                             ▼
┌─────────────────────────────────────────────────────────────┐
│              OpenClaw Gateway(LLM Runtime)                 │
│                                                              │
│  - 提供 auth / model routing / provider dispatch            │
│  - 路由到指定 agent:contentpipe-blank                     │
│  - 管理 session 上下文(per run / per node 隔离)           │
│  - 支持模型 override:scout/writer/director 等走不同模型    │
│                                                              │
│  OpenClaw 在这里是"能力平台",不是"调用方"                  │
└─────────────────────────────────────────────────────────────┘
```

**集成方向说明**

- ContentPipe **调用** OpenClaw Gateway
- 不是 OpenClaw 调用 ContentPipe
- ContentPipe 是**外部 Python/FastAPI 服务**,独立运行
- OpenClaw Gateway 在这里充当**可路由的 LLM runtime**
- blank-agent(`contentpipe-blank`)是 ContentPipe 专用的 agent 标识,用于:
  - 隔离 ContentPipe 的 LLM 调用上下文
  - 绑定 skill 边界(skills 来自 `plugins/content-pipeline/skills/`)
  - 支持 per-node / per-run 的 session key 隔离

### 2.1 blank-agent 执行平面(v0.7.1+)

ContentPipe 通过 OpenClaw Gateway 调用 LLM 时,使用 `contentpipe-blank` 作为 agent 标识:
...
Headers:
  ...
  X-OpenClaw-Agent-Id: contentpipe-blank
  X-OpenClaw-Session-Key: contentpipe:{run_id}:{node}:main
  X-OpenClaw-Model: {effective_model}    # override 时使用

Body:
  model: "openclaw/contentpipe-blank"      # 或 "openclaw/default"
  messages: [...]
```

**blank-agent 的职责**

- 承载节点的**正式产物修改**(初始执行 + 后续审核追问)
- 节点收到命令后先自行执行任务并尝试修改自己的正式产物文件
- Python 在**每一轮 LLM 运行后**读回文件,并先做最小提交判定
...
- 生效方式:`openclaw gateway restart`(配置改动后需要)
- 正式产物路径:`plugins/content-pipeline/output/runs/<run_id>/`

**重要约束**

- ContentPipe **不是** OpenClaw 原生 plugin
- `plugins/content-pipeline/` 目录**不需要**被 Gateway 作为 plugin 加载
- 目录中的 `skills/` 通过 workspace `skills.load.extraDirs` 对 agent 可见
- 唯一需要的 OpenClaw 接入点是 Gateway 的 HTTP API

### 2.2 内置 skill 单源架构

ContentPipe 的能力增强以内置 skills 形式提供，不依赖外部安装：
...
skills 通过 OpenClaw workspace 全局 `skills.load.extraDirs` 配置生效：

```json
// openclaw.json（workspace 级）
{
  "skills": {
    "load": {
      "extraDirs": [
        "/home/ssp/work/openclawWS/clawdbot_workspace/skills",
        "/home/ssp/work/openclawWS/clawdbot_workspace/plugins/content-pipeline/skills"
      ]
    }
  }
}
...
- `contentpipe-blank` agent 默认继承 workspace skill 路径
- 若需隔离，可显式设置 `"skills": []` 关闭（`vidpipe-blank` 采用了这种方式）

**设计约束**

- skills 作为 ContentPipe 内容的一部分，**与仓库版本一起发布**
- 不采用"仓库内置 + 运行时外装"的双轨制
...
- 无需 plugin 注册，skills 通过 workspace extraDirs 自动生效
